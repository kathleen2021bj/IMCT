...
parser.add_argument('-nc', '--num_of_clients', type=int, default=90, help='numer of the clients')
parser.add_argument('-cf', '--cfraction', type=float, default=1, help='C fraction, 0 means 1 client, 1 means total clients')
parser.add_argument('-E', '--epoch', type=int, default=4, help='local train epoch')
...
if __name__=="__main__":
    ...
    sys.stdout = Logger('!!eyi_model_fedprox80ceshi90.txt')
...
#     global_parameters = torch.load("./model/model5")
#
# # model1 0.92
# # 使用上一次model5 0.6233999729156494的模型
#     init_global_accu = 0
#     init_global_num = 0
...
#         init_global_accu += (preds == label).float().mean()
#         init_global_num += 1
#     print('init_global_accuracy: {}'.format(init_global_accu / init_global_num))
    ...
    for i in range(args['num_comm']):
        if i != 200:
            ...
            for client in tqdm(clients_in_comm):
                ...
                list_jiazong[j % 90].append(local_accuracy)
                j += 1
            ...
            model_eyi = torch.load("./model/model86")
            for var in global_parameters:
                global_parameters[var] = (sum_parameters[var] / 90)
            ...
            #
            #     # 判断，中位值与平均值相差超过0.01就减去1个var，不超过就减去3个var
